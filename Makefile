...
.PHONY: help setup init-secrets edit-secrets test-secrets test-email up down restart start stop status health health-email logs logs-tail logs-postfix backup backup-full backup-emergency list-backups restore restore-db update update-system maintenance maintenance-full update-dns breakglass-create breakglass-status breakglass-remove cron-install cron-remove cron-list dev-setup test test-config dry-run db-maint db-backup clean clean-all prune info shell version watch monitor safe-restart fmt config
...
setup: ## Run initial setup (requires sudo)
	...
	@if [ "$$(id -u)" -eq 0 ]; then echo "$(RED)Error: Run as normal user via 'sudo make setup' or use setup.sh$(NC)"; exit 1; fi
...
test-secrets: ## Test secrets decryption
	...
	@./edit-secrets.sh --list > /dev/null && echo "$(GREEN)Secrets test passed$(NC)" || echo "$(RED)Secrets test failed$(NC)"
...
up: ## Start all services with secrets initialization
	...
	@./startup.sh || { echo "$(RED)Startup failed!$(NC)"; $(MAKE) status; exit 1; }
...
restart: ## Restart all services (enhanced startup)
	...
	@./startup.sh --force-restart || { echo "$(RED)Restart failed!$(NC)"; $(MAKE) status; exit 1; }
...
backup: ## Create backup (TYPE: db, full, emergency)
	...
	@./backup.sh --type $(if $(TYPE),$(TYPE),db) $(if $(TYPE),--email,)
...
test: ## Run all tests
	...
	@$(MAKE) test-secrets
	@$(MAKE) test-email
	@$(DOCKER_COMP) config > /dev/null && echo "$(GREEN)Docker Compose config valid$(NC)" || echo "$(RED)Docker Compose config invalid$(NC)"
...
version: ## Show version information
	...
	@echo "$(GREEN)Docker Compose:$(NC) $$($(DOCKER_COMP) version)"
