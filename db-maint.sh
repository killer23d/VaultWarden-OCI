...
main() {
    ...
    if run_sqlite_sidecar "PRAGMA integrity_check;" | grep -q "ok"; then
    ...
    if run_sqlite_sidecar "PRAGMA wal_checkpoint(TRUNCATE);"; then
    ...
    if run_sqlite_sidecar "PRAGMA optimize;"; then
    ...
    if run_sqlite_sidecar "VACUUM;"; then
