...
if [[ "$CREATE_USER" == "false" && "$REMOVE_USER" == "false" && "$RESET_PASSWORD" == "false" && "$SHOW_STATUS" == "false" ]]; then
    log_error "Must specify --create, --remove, --reset-password, or --status"
    show_help
    exit 1
fi
...
create_breakglass_user() {
    ...
    local password
    if ! password=$(generate_breakglass_password); then
        return 1
    fi
    ...
    if ! cat > "$instructions_file" << EOF; then
...
EOF
        log_warn "Failed to create instructions file"
    else
        chown "$BREAKGLASS_USER:$BREAKGLASS_USER" "$instructions_file"
...
remove_breakglass_user() {
    ...
    if [[ -f "$sudoers_file" ]]; then
        if rm -f "$sudoers_file"; then
            ...
            log_warn "Failed to remove sudoers configuration"
        fi
    fi
...
# STANDARDIZED: Show break-glass status - returns exit code
show_breakglass_status() {
    ...
    if check_user_exists; then
        ...
        if user_info=$(getent passwd "$BREAKGLASS_USER"); then
            ...
            if [[ -f "$home_dir/EMERGENCY_ACCESS_INSTRUCTIONS.txt" ]]; then
...
main() {
    ...
    if [[ "$CREATE_USER" == "true" ]]; then
        if create_breakglass_user; then
            ...
            echo "  4. Disable when not needed: sudo ./create-breakglass-admin.sh --remove"
