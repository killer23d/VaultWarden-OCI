...
class UI:
    ...
    def _line(self, label: str, text: str, code: str, *, file=None) -> None:
        prefix = f"\033[{code}m{label}\033[0m" if self.color else label
        print(f"{prefix} {text}", file=file or sys.stdout)
    def header(self, text: str) -> None: print(f"\n== {text} ==")
    def ok(self, text: str, *, file=None) -> None: self._line("PASS", text, "32", file=file)
    def warn(self, text: str, *, file=None) -> None: self._line("WARN", text, "33", file=file)
    def info(self, text: str, *, file=None) -> None: self._line("INFO", text, "36", file=file)
    def action(self, text: str, *, file=None) -> None: self._line("ACTION", text, "34", file=file)
    def fail(self, text: str, *, file=None) -> None: self._line("FAIL", text, "31", file=file)
...
#
# Cloudflare account/zone IDs and the CrowdSec local LAPI bouncer credential are
# discovered/generated by the appliance; the operator must not copy legacy IDs/tokens here.
schema_version = 1
