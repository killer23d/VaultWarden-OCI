...
All configuration is split between two files: **`.env`** (non-sensitive settings) and **the configured secrets file** (`secrets/secrets.yaml` by default; encrypted with Age + SOPS and edited via `./utilities/secrets-edit.sh`). Both are generated from `.example` templates by `setup.sh` — never edit generated files directly.
