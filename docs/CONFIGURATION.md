...
All configuration is split between two files: **`.env`** (non-sensitive settings) and **`$SECRETS_FILE`** (encrypted with Age + SOPS, edited via `./utilities/secrets-edit.sh`). Both are generated from `.example` templates by `setup.sh` — never edit generated files directly.
