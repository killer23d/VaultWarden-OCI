...
| `fail2ban_cloudflare_firewall_token` | CrowdSec cloudflare-bouncer edge banning (Zone:Firewall Services:Edit). Read by `setup.sh` from disk during installation. | [Cloudflare API Tokens](https://dash.cloudflare.com/profile/api-tokens) |
...
> **Note:** `fail2ban_cloudflare_firewall_token` is used by CrowdSec cloudflare-bouncer. Without it, CrowdSec can detect attacks but cannot push bans to Cloudflare WAF.
