...
| :-- | :-- | :-- |
| `admin_basic_auth_hash` | Bcrypt hash for Caddy `/admin` basic auth | Auto-generated by `setup.sh --auto`. To regenerate: `./setup.sh --phase=secrets --hash-only` |
...
# setup.sh --phase=systemd --install copies the key here so systemd units
...
Scheduled jobs are managed by systemd timers installed by `setup.sh --phase=systemd`:
