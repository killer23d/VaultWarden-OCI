...
> **💡 If you ran `setup.sh install --auto`:** `admin_token` and `admin_basic_auth_hash` were already generated by `setup.sh secrets --auto`. Run `./edit-secrets.sh view` to confirm them. You still need to set `caddy_cloudflare_dns_token` and `fail2ban_cloudflare_firewall_token` — those cannot be auto-generated.
...
> **⚠️ Literal values required.** `.env` values are read literally by the stack. `F2B_DEST_MAIL`, `F2B_SENDER`, and the `VW_SMTP_*` block must be real values, not references to other variables.
...
| **postfix** | SMTP sidecar for VaultWarden, Fail2Ban, and SMTP fallback | 256 MB |
...
./edit-secrets.sh edit   # verify the API token key for EMAIL_PROVIDER is set
```
