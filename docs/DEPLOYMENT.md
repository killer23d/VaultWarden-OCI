...
> **💡 If you ran `setup.sh --auto`:** `admin_token` and `admin_basic_auth_hash` were already generated by `setup.sh --phase=secrets --auto`. Run `./edit-secrets.sh --view` to confirm them. You still need to set `caddy_cloudflare_dns_token` and `fail2ban_cloudflare_firewall_token` — those cannot be auto-generated.
...
Set at minimum:
- `admin_basic_auth_hash` — auto-generated by `setup.sh --phase=secrets --auto`; to regenerate manually:
  ```bash
...
> **Note:** `cron-setup.sh` has been removed from the repository. Use `setup.sh --phase=systemd --install` to install all scheduled jobs. If you have legacy cron jobs from a prior installation, remove them to avoid duplicate runs.
...
- ✅ Validate systemd timer installation: `sudo ./setup.sh --phase=systemd --validate`
- ✅ Confirm timers are running: `sudo ./setup.sh --phase=systemd --status`
...
```bash
sudo ./setup.sh --phase=systemd --status              # list all VaultWarden timers
sudo ./setup.sh --phase=systemd --validate            # check for split-brain / missing files
