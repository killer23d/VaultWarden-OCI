...
> **💡 If you ran `setup.sh --auto`:** `admin_token` and `admin_basic_auth_hash` were already generated by `setup.sh secrets --auto`. Run `./edit-secrets.sh view` to confirm them. You still need to set `caddy_cloudflare_dns_token` and `fail2ban_cloudflare_firewall_token` — those cannot be auto-generated.
