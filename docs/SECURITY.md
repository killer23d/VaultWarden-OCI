...
Vaultwarden's upstream Admin UI normally persists edits in `DATA_FOLDER/config.json`, and that user file takes precedence over environment configuration. The steady-state appliance must not allow that file to become a second durable authority, but an upgrade also must not silently discard policy that was already saved there. A fresh or explicitly reconciled installation therefore uses `CONFIG_FILE=/tmp/vaultwarden-admin-config.json` on container tmpfs. If an upgrade finds a pre-existing `/data/config.json`, the candidate deliberately keeps that historical file effective until the operator reconciles appliance-supported values through the validated config/SOPS editors and explicitly finalizes the transition. The finalization marker is root-only and bound to the SHA-256 of that exact historical file; the file is retained for evidence rather than deleted. A later modification invalidates the marker and makes reconciliation pending again. Unsupported/incompatible/sensitive legacy key **names** are shown before finalization, but their values are not printed.
...
A pre-existing Vaultwarden Admin `config.json` is a bounded upgrade-compatibility exception to the single-authority steady state. Keeping it effective until explicit reconciliation is safer than silently replacing its policy. It must not become an open-ended second config framework: once supported values are reconciled and the operator explicitly acknowledges legacy-only/sensitive keys, the next start/restart returns to tmpfs Admin persistence and `config.toml`/SOPS-only durable authority.
