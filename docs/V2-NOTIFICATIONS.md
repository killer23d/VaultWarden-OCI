...
- `vaultwarden-oci-health.service` + `.timer` — runs `vwctl status` every five minutes; notification-delivery history is advisory and by itself cannot fail or re-notify this unit.
