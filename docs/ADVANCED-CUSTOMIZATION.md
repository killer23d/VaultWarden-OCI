...
Email delivery is handled by **`lib/email.sh`** — a pure bash + curl multi-provider chain. SMTP can go straight to your relay or through the Postfix sidecar; `host` mode is the final local mail/sendmail fallback.
