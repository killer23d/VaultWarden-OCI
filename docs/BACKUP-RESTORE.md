...
> **⚠️ Age key mismatch — silent data-loss risk:** Secrets inside an emergency backup were encrypted with the Age key that was **active at backup time**. If you restore an emergency kit onto a new server that has a *different* Age key (for example, one generated by a fresh `setup.sh` run before the restore), the secrets inside the archive will silently fail to decrypt. Always restore the Age key from inside the emergency kit *before* running `setup.sh install --force`, or use `./restore.sh latest --file <archive>` which handles key placement automatically. See [BOOTSTRAP_KEY_RECOVERY.md](BOOTSTRAP_KEY_RECOVERY.md) for the full key recovery procedure.
...
#    pass it directly with RESTORE_AGE_KEY_FILE.
RESTORE_AGE_KEY_FILE=/path/to/age-key-from-emergency-kit.txt \
