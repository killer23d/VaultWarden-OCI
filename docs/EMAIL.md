...
`EMAIL_MODE=host` first tries a local sendmail/mail binary on the host. If
that path is unavailable, the shared email dispatcher falls back to SMTP.
Attachment delivery requires `sendmail` for the host-MTA stage because the
message must preserve its multipart MIME structure.
