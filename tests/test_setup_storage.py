...
class StorageContractTests(unittest.TestCase):
    ...
    def test_conflicting_rerun_fails_before_fstab_or_identity_mutation(self) -> None:
        mounted = storage.StorageIdentity("aaaa-bbbb", "ext4", "/dev/vdb"); selected = storage.StorageIdentity("cccc-dddd", "ext4", "/dev/vdc")
        with mock.patch.object(storage, "_real_device", return_value="/dev/vdc"), mock.patch.object(storage, "reject_boot_related"), mock.patch.object(storage, "_blkid", return_value="ext4"), mock.patch.object(storage, "_signature_types", return_value={"ext4"}), mock.patch.object(storage, "_selected_identity", return_value=selected), mock.patch.object(storage.Path, "mkdir"), mock.patch.object(storage.Path, "is_mount", return_value=True), mock.patch.object(storage, "_identity_from_mount", return_value=mounted), mock.patch.object(storage, "ensure_fstab") as fstab, mock.patch.object(storage, "write_identity") as identity:
            with self.assertRaisesRegex(storage.StorageError, "refusing to rewrite"): storage.provision("/dev/vdc", acknowledge_existing=True, runner=lambda argv: result(argv, "disk\n"))
...
class SetupContractTests(unittest.TestCase):
    ...
    def test_generated_config_is_parsed_by_canonical_runtime_owner(self) -> None:
        setup._validate_config_text(setup._config_text("vault.example.com", "admin@example.com", "age1" + "q" * 58))
    ...
    def test_config_rerun_is_idempotent_and_does_not_overwrite_operator_changes(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(setup, "CONFIG", path):
                setup._ensure_config("vault.example.com", "admin@example.com", offline); path.write_text(desired.replace("signups_allowed = false", "signups_allowed = true"))
    ...
    def test_all_persistent_state_commands_fail_before_cli_when_storage_missing(self) -> None:
        for command in ("start", "backup", "restore", "recovery", "edge", "crowdsec", "notify"):
            with self.subTest(command=command), mock.patch.object(update_cli.storage, "verify", side_effect=storage.StorageError("missing mount")), mock.patch.object(update_cli.cli, "main") as delegated:
                self.assertEqual(update_cli.main([command]), 1); delegated.assert_not_called()
    ...
    def test_doctor_storage_id_is_in_canonical_order_without_ansi(self) -> None:
        ...
        with mock.patch.object(update_cli.cli, "doctor_checks", return_value=base), mock.patch.object(update_cli.storage, "verify", side_effect=storage.StorageError("missing mount")), mock.patch("builtins.print") as printer:
            self.assertEqual(update_cli.main(["doctor"]), 1)
        rendered = "\n".join(" ".join(str(value) for value in call.args) for call in printer.call_args_list); self.assertIn("storage.dedicated", rendered); self.assertNotIn("\x1b[", rendered)
