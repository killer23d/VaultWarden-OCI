#!/usr/bin/env python3
"""Prove the supported predecessor updater can consume this source tree."""
...
# CI intentionally supplies PYTHONPATH for the immutable supported predecessor
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: acceptance_previous_update.py CANDIDATE_SOURCE")
    ...
    if candidate_manifest.version == predecessor_manifest.version:
        raise SystemExit(
            "candidate reuses the supported predecessor immutable release identity: "
            f"{candidate_manifest.version}"
        )

    # This is the exact pre-candidate-code source gate used by the supported
    # predecessor updater. It must accept the release source before new code runs.
    ...
    with tempfile.TemporaryDirectory(prefix="vwoci-previous-updater-") as directory:
        ...
        units = installed / install.SYSTEMD_SOURCE_DIR
        if not units.is_dir():
            raise SystemExit("predecessor updater did not stage the canonical systemd source layout")
        missing = [unit for unit in install.SYSTEMD_UNITS if not (units / unit).is_file()]
        if missing:
    ...
    print(
        "PASS: supported predecessor updater accepts and stages candidate source "
