...
grep -Fq '_print_backup_run_summary "$actual_type" "$backup_file" "$verification_status" "$offsite_status"' "$BACKUP" || fail "backup final summary call missing"
grep -Fq '[[ "$QUIET" == "true" ]] && return 0' "$BACKUP" || fail "backup summary is not quiet-aware"

verify_failure_block="$(awk '/if ! verify_backup_quick/,/exit 1/' "$BACKUP")"
grep -Fq 'if [[ "$RCLONE_SYNC" == "true" ]]; then' <<< "$verify_failure_block" || fail "backup verification failure should not change offsite status when rclone was not requested"
grep -Fq 'offsite_status="skipped because verification failed"' <<< "$verify_failure_block" || fail "backup verification failure missing rclone-specific offsite skip state"
grep -Fq 'rm -f "$backup_file" "${backup_file}.meta" "${backup_file}.sha256" "${backup_file}.sha256.hmac"' <<< "$verify_failure_block" || fail "backup verification failure does not discard archive and sidecars"
...
grep -qi 'emergency offsite delivery is incomplete\|remote emergency recovery point is incomplete' "$TMP/meta-fail.out" || fail 'emergency .meta upload failure must report incomplete recovery delivery'
! grep -q 'The primary backup archive was delivered' "$TMP/meta-fail.out" || fail 'emergency .meta upload failure must not claim the primary is a complete delivered recovery point'
...
grep -q 'The primary backup archive was delivered' "$TMP/sha-fail.out" || fail '.sha256 upload failure must retain warning-class primary-delivered wording'

for meta_case in zero missing-mode unsupported duplicate; do
  : > "$TMP/rclone-copy.calls"; rm -f "$TMP/age.called"
  META_CASE="$meta_case" bash "$TMP/sync-probe.sh" >"$TMP/$meta_case.out" 2>&1 || fail "emergency metadata $meta_case probe crashed"
  grep -q '^RC=3$' "$TMP/$meta_case.out" || { cat "$TMP/$meta_case.out" >&2; fail "emergency metadata $meta_case must return distinct status 3"; }
  ! grep -q 'Offsite sync complete' "$TMP/$meta_case.out" || fail "emergency metadata $meta_case printed complete offsite wording"
  ! grep -q 'The primary backup archive was delivered' "$TMP/$meta_case.out" || fail "emergency metadata $meta_case claimed the remote primary was delivered as a complete recovery point"
  grep -qi 'restore-critical metadata is missing, unusable, or not delivered' "$TMP/$meta_case.out" || fail "emergency metadata $meta_case did not explain unusable restore-critical metadata"
...
for meta_case in age-passphrase age-recipient; do
  : > "$TMP/rclone-copy.calls"
  META_CASE="$meta_case" bash "$TMP/sync-probe.sh" >"$TMP/$meta_case.out" 2>&1 || fail "supported emergency metadata $meta_case probe crashed"
...
printf archive > "\$archive"; printf checksum > "\${archive}.sha256"
case "\${VERIFY_META_CASE:-missing-mode}" in
  missing-mode) printf 'version=2\\n' > "\${archive}.meta" ;;
  unsupported) printf 'encryption_mode=future-mode\\n' > "\${archive}.meta" ;;
  *) exit 91 ;;
esac
quick_rc=0; verify_backup_quick "\$archive" "$TMP/key.txt" emergency || quick_rc=\$?
full_rc=0; verify_backup_full "\$archive" emergency "$TMP" || full_rc=\$?
printf 'QUICK_RC=%s\\nFULL_RC=%s\\n' "\$quick_rc" "\$full_rc"
EOF_PROBE
for verify_case in missing-mode unsupported; do
  : > "$TMP/verify-age.calls"
  VERIFY_META_CASE="$verify_case" bash "$TMP/verify-metadata-probe.sh" >"$TMP/verify-$verify_case.out" 2>&1 || fail "emergency verification metadata $verify_case probe crashed"
...
rm -rf "\$BASE_DIR"; mkdir -p "\$BASE_DIR/emergency"
...
if BATCH_META_CASE=zero bash "$TMP/batch-sync-probe.sh" >"$TMP/batch-invalid.out" 2>&1; then fail 'standalone retained sync accepted zero-byte emergency metadata'; fi
grep -Fq 'emergency-retained-20260710-120000.tar.zst.age' "$TMP/batch-invalid.out" || fail 'standalone retained sync did not identify the invalid emergency archive'
...
: > "$TMP/batch-copy.calls"
BATCH_META_CASE=valid bash "$TMP/batch-sync-probe.sh" >"$TMP/batch-valid.out" 2>&1 || { cat "$TMP/batch-valid.out" >&2; fail 'standalone retained sync rejected supported emergency metadata'; }
...
grep -Fq 'elif (( _sync_rc == 3 )); then' "$BACKUP" || fail 'backup run caller must handle emergency metadata delivery status separately'
grep -Fq 'FAILED: emergency restore metadata missing or unusable; local backup is safe' "$BACKUP" || fail 'backup run summary must mark incomplete emergency offsite delivery as failed'
...
[[ "${result_arr[1]:-}" == "$(realpath -e "$mock_cfg")" ]] || fail "resolver returned the wrong explicit config path"
...
[[ -n "$canonical_line" && -n "$exception_line" && "$canonical_line" -lt "$exception_line" ]] || fail "root rclone exception must be evaluated only after canonical path resolution"
grep -Fq '&& "$prefix" == "/root"' "$UTILS" || fail "root rclone validator exception is not narrowly scoped to /root"
...
root_dir=/root/.config/rclone; root_parent=/root/.config; root_config="${root_dir}/rclone.conf"; unrelated="${root_dir}/vw-rclone-unrelated-$$.conf"
created_root_dir=false; created_root_parent=false
[[ ! -e "$root_config" && ! -L "$root_config" ]] || exit 77
[[ ! -e /etc/rclone/rclone.conf && ! -L /etc/rclone/rclone.conf ]] || exit 77
unset SUDO_USER
cleanup(){ rm -f "$unrelated" "$root_config" "${root_config}.real"; [[ "$created_root_dir" == true ]] && rmdir "$root_dir" 2>/dev/null || true; [[ "$created_root_parent" == true ]] && rmdir "$root_parent" 2>/dev/null || true; }
trap cleanup EXIT
if [[ ! -d "$root_dir" ]]; then [[ -d "$root_parent" ]] || created_root_parent=true; install -d -m 700 "$root_dir"; created_root_dir=true; fi
printf '[test]\ntype = local\n' > "$root_config"; chmod 600 "$root_config"; RCLONE_CONFIG=""
resolved="$(_resolve_rclone_config)"; [[ "$resolved" == "$root_config" ]] || exit 1; validate_rclone_config_path "$resolved" || exit 1
printf '[test]\ntype = local\n' > "$unrelated"; chmod 600 "$unrelated"
if validate_rclone_config_path "$unrelated"; then exit 1; fi
if validate_rclone_config_path /etc/shadow; then exit 1; fi
mv "$root_config" "${root_config}.real"; ln -s /etc/shadow "$root_config"
if validate_rclone_config_path "$root_config"; then exit 1; fi
rm -f "$root_config"; mv "${root_config}.real" "$root_config"; chmod 666 "$root_config"
if validate_rclone_config_path "$root_config"; then exit 1; fi
chmod 600 "$root_config"
if id -u nobody >/dev/null 2>&1; then chown "$(id -u nobody)" "$root_config"; if validate_rclone_config_path "$root_config"; then exit 1; fi; chown 0 "$root_config"; fi
EOF_ROOT_PROBE
chmod 700 "$root_probe"

root_probe_rc=0
if (( EUID == 0 )); then PROJECT_ROOT="$ROOT" "$BASH" "$root_probe" || root_probe_rc=$?
elif command -v sudo >/dev/null 2>&1 && sudo -n true >/dev/null 2>&1; then sudo -n env PROJECT_ROOT="$ROOT" "$BASH" "$root_probe" || root_probe_rc=$?
else root_probe_rc=77; fi

if (( root_probe_rc == 0 )); then printf 'PASS: canonical root rclone resolver/validator contract\n'
elif (( root_probe_rc == 77 )); then printf 'SKIP: root rclone fixture unavailable; structural canonical-path assertions passed\n'
else fail "canonical root rclone config probe failed (exit ${root_probe_rc})"; fi
...
_write_archive_with_sidecars(){ local archive="$1"; printf 'archive\n' > "$archive"; printf 'sha\n' > "$archive.sha256"; printf 'hmac\n' > "$archive.sha256.hmac"; printf 'meta\n' > "$archive.meta"; }
_assert_exists(){ [[ -e "$1" ]] || fail "expected retained file missing: $1"; }
_assert_missing(){ [[ ! -e "$1" ]] || fail "expected deleted file still present: $1"; }
...
get_config_value(){ local key="$1" fallback="${2:-}"; printf '%s\n' "${RETENTION_CONFIG[$key]:-$fallback}"; }
...
[[ "$(backup_retention_days_for_type db 7)" == 7 ]] || fail '--keep explicit override did not win'
...
[[ "$(backup_retention_days_for_type db)" == 14 ]] || fail 'safe 14-day fallback was not selected'
for invalid in 0 -1 abc 1.5; do
  if backup_retention_days_for_type db "$invalid" >/dev/null 2>&1; then fail "invalid explicit retention accepted: $invalid"; fi
done
RETENTION_CONFIG[BACKUP_RETENTION_DB_DAYS]=bad
if backup_retention_days_for_type db >/dev/null 2>&1; then fail 'invalid configured retention was accepted'; fi
if backup_retention_days_for_type unknown >/dev/null 2>&1; then fail 'unknown backup type was accepted'; fi

for file in "$ROOT/.env.example" "$ROOT/docs/CONFIGURATION.md" "$ROOT/docs/ADVANCED-CUSTOMIZATION.md"; do
  ! grep -Eq 'BACKUP_ENCRYPTION_ENABLED|BACKUP_VERIFICATION_MODE' "$file" || fail "unowned backup setting remains in $file"
done
if grep -R -nE 'BACKUP_ENCRYPTION_ENABLED|BACKUP_VERIFICATION_MODE' "$ROOT" --include='*.sh' --include='*.bash' --exclude-dir=.git --exclude-dir=tests >/dev/null; then
...
grep -Fq 'backup_retention_days_for_type "$backup_type"' "$MAINT" || fail 'maintenance does not use canonical retention resolver'
grep -Fq 'backup_retention_days_for_type "$actual_type" "${KEEP_DAYS:-}"' "$BACKUP" || fail 'backup run does not preserve explicit --keep precedence'
grep -Fq 'Local retention completed, but remote retention failed.' "$BACKUP" || fail 'rotate does not distinguish local completion from remote failure'

local_dir="$TMP/local/db"; mkdir -p "$local_dir"
newest="$local_dir/db-a-20010101-000000.age"; older="$local_dir/db-z-20000101-000000.age"
_write_archive_with_sidecars "$newest"; _write_archive_with_sidecars "$older"
cleanup_old_backups "$local_dir" db 1 > "$TMP/local-retention.out" 2>&1 || { cat "$TMP/local-retention.out" >&2; fail "local retention cleanup failed"; }
for suffix in "" ".sha256" ".sha256.hmac" ".meta"; do _assert_exists "$newest$suffix"; _assert_missing "$older$suffix"; done

unparseable="$local_dir/db-manual-before-contract.age"; _write_archive_with_sidecars "$unparseable"
cleanup_old_backups "$local_dir" db 1 > "$TMP/local-unparseable.out" 2>&1 || { cat "$TMP/local-unparseable.out" >&2; fail "local retention with unparseable archive failed"; }
_assert_exists "$unparseable"

no_parse_dir="$TMP/no-parse/db"; mkdir -p "$no_parse_dir"
no_parse_a="$no_parse_dir/db-manual-a.age"; no_parse_b="$no_parse_dir/db-manual-b.age"
_write_archive_with_sidecars "$no_parse_a"; _write_archive_with_sidecars "$no_parse_b"
cleanup_old_backups "$no_parse_dir" db 1 > "$TMP/local-no-parse.out" 2>&1 || { cat "$TMP/local-no-parse.out" >&2; fail "local no-parse retention failed"; }
_assert_exists "$no_parse_a"; _assert_exists "$no_parse_b"

dry_run_dir="$TMP/dry-run/db"; mkdir -p "$dry_run_dir"
dry_run_older="$dry_run_dir/db-z-20000101-000000.age"; dry_run_newest="$dry_run_dir/db-a-20010101-000000.age"; orphan_sidecar="$dry_run_dir/db-orphan-19990101-000000.age.meta"
_write_archive_with_sidecars "$dry_run_older"; _write_archive_with_sidecars "$dry_run_newest"; printf 'orphan\n' > "$orphan_sidecar"
DRY_RUN=true cleanup_old_backups "$dry_run_dir" db 1 > "$TMP/local-dry-run.out" 2>&1 || { cat "$TMP/local-dry-run.out" >&2; fail "local retention dry-run failed"; }
for file in "$dry_run_older" "$dry_run_older.sha256" "$dry_run_older.sha256.hmac" "$dry_run_older.meta" "$dry_run_newest" "$dry_run_newest.sha256" "$dry_run_newest.sha256.hmac" "$dry_run_newest.meta" "$orphan_sidecar"; do _assert_exists "$file"; done
grep -Fq "[DRY RUN] Would remove: $(basename "$dry_run_older") (and sidecars)" "$TMP/local-dry-run.out" || fail "local dry-run did not report stale deletion candidate"
! grep -Fq "[DRY RUN] Would remove: $(basename "$dry_run_newest") (and sidecars)" "$TMP/local-dry-run.out" || fail "local dry-run reported newest archive as deletion candidate"
grep -Fq '[DRY RUN] Would clean up 1 old db backups' "$TMP/local-dry-run.out" || fail "local dry-run summary did not count planned deletion"
grep -Fq "[DRY RUN] Would remove orphaned sidecar: $(basename "$orphan_sidecar")" "$TMP/local-dry-run.out" || fail "local dry-run did not report orphaned sidecar"

source "$MAINT"
maintenance_dry_run_dir="$TMP/maintenance-dry-run"; mkdir -p "$maintenance_dry_run_dir/db"
maintenance_older="$maintenance_dry_run_dir/db/db-z-20000101-000000.age"; maintenance_newest="$maintenance_dry_run_dir/db/db-a-20010101-000000.age"; maintenance_manual="$maintenance_dry_run_dir/db/db-manual-before-contract.age"; maintenance_orphan="$maintenance_dry_run_dir/db/db-orphan-19990101-000000.age.meta"
_write_archive_with_sidecars "$maintenance_older"; _write_archive_with_sidecars "$maintenance_newest"; _write_archive_with_sidecars "$maintenance_manual"; printf 'orphan\n' > "$maintenance_orphan"
...
DRY_RUN=true CLEAN_BACKUPS=true cleanup_backups > "$TMP/maintenance-dry-run.out" 2>&1 || { cat "$TMP/maintenance-dry-run.out" >&2; fail "maintenance retention dry-run failed"; }
for file in "$maintenance_older" "$maintenance_older.sha256" "$maintenance_older.sha256.hmac" "$maintenance_older.meta" "$maintenance_newest" "$maintenance_newest.sha256" "$maintenance_newest.sha256.hmac" "$maintenance_newest.meta" "$maintenance_manual" "$maintenance_manual.sha256" "$maintenance_manual.sha256.hmac" "$maintenance_manual.meta" "$maintenance_orphan"; do _assert_exists "$file"; done
grep -Fq "[DRY RUN] Would clean up db backups older than 1 days" "$TMP/maintenance-dry-run.out" || fail "maintenance did not use canonical db retention"
grep -Fq 'db backup retention preview completed (1d retention)' "$TMP/maintenance-dry-run.out" || fail "maintenance did not report retention preview"
! grep -Fq 'db backups cleaned (1d retention)' "$TMP/maintenance-dry-run.out" || fail "maintenance dry-run falsely reported deletion"
! grep -Fq "[DRY RUN] Would remove: $(basename "$maintenance_newest") (and sidecars)" "$TMP/maintenance-dry-run.out" || fail "maintenance reported newest archive as deletion candidate"
! grep -Fq "[DRY RUN] Would remove: $(basename "$maintenance_manual") (and sidecars)" "$TMP/maintenance-dry-run.out" || fail "maintenance reported manual archive as deletion candidate"

if (( BASH_VERSINFO[0] < 5 )); then printf 'SKIP: remote prune mock requires Bash 5 namerefs used by production backup-run.sh\n'; return 0; fi
...
REMOTE_MODE="\${REMOTE_MODE:-normal}"
KEEP_DAYS=""
backup_log_info(){ printf 'INFO:%s\n' "\$*" >> "\$REMOTE_LOG"; }
backup_log_success(){ printf 'SUCCESS:%s\n' "\$*" >> "\$REMOTE_LOG"; }
backup_log_warn(){ printf 'WARN:%s\n' "\$*" >> "\$REMOTE_LOG"; }
...
rclone(){
  ...
  case "\$cmd" in
    lsf)
      if [[ "\$REMOTE_MODE" == fail ]]; then printf 'simulated listing failure\n' >&2; return 17; fi
      if [[ "\$REMOTE_MODE" == empty ]]; then return 0; fi
      local arg
      for arg in "\$@"; do
        if [[ "\$arg" == "mockremote:vaultwarden_backups/db/" ]]; then
          printf '%s\n' 'db-z-20000101-000000.age' 'db-manual-upload.age' 'db-a-20010101-000000.age'; return 0
        fi
      done
      return 0
      ;;
...
: > "$TMP/remote-delete.log"; : > "$TMP/remote-prune.log"
REMOTE_DRY_RUN=true REMOTE_MODE=normal bash "$TMP/remote-prune-probe.sh" || { cat "$TMP/remote-prune.log" >&2; fail "remote dry-run prune probe failed"; }
[[ ! -s "$TMP/remote-delete.log" ]] || fail "remote dry-run called rclone deletefile"
grep -Fq 'Would delete remote: mockremote:vaultwarden_backups/db/db-z-20000101-000000.age' "$TMP/remote-prune.log" || fail "remote dry-run did not report stale archive"
! grep -Fq 'Would delete remote: mockremote:vaultwarden_backups/db/db-a-20010101-000000.age' "$TMP/remote-prune.log" || fail "remote dry-run reported newest archive"

: > "$TMP/remote-delete.log"; : > "$TMP/remote-prune.log"
REMOTE_MODE=normal bash "$TMP/remote-prune-probe.sh" || { cat "$TMP/remote-prune.log" >&2; fail "remote prune probe failed"; }
primary_line=$(grep -nF 'mockremote:vaultwarden_backups/db/db-z-20000101-000000.age --contimeout' "$TMP/remote-delete.log" | head -1 | cut -d: -f1)
[[ -n "$primary_line" ]] || fail "remote prune did not delete stale primary"
for suffix in .sha256 .sha256.hmac .meta; do
  sidecar_line=$(grep -nF "mockremote:vaultwarden_backups/db/db-z-20000101-000000.age${suffix}" "$TMP/remote-delete.log" | head -1 | cut -d: -f1)
  [[ -n "$sidecar_line" ]] || fail "remote prune did not delete sidecar ${suffix}"
  (( primary_line < sidecar_line )) || fail "remote sidecar ${suffix} was deleted before its primary"
done
! grep -Fq 'mockremote:vaultwarden_backups/db/db-a-20010101-000000.age --contimeout' "$TMP/remote-delete.log" || fail "remote prune deleted newest archive"
! grep -Fq 'mockremote:vaultwarden_backups/db/db-manual-upload.age --contimeout' "$TMP/remote-delete.log" || fail "remote prune deleted unparseable archive"

: > "$TMP/remote-delete.log"; : > "$TMP/remote-prune.log"
REMOTE_MODE=empty bash "$TMP/remote-prune-probe.sh" || { cat "$TMP/remote-prune.log" >&2; fail "empty remote inventory should succeed"; }
[[ ! -s "$TMP/remote-delete.log" ]] || fail "empty remote inventory triggered deletion"
grep -Fq '[remote] No db backup archives found on remote — nothing to prune.' "$TMP/remote-prune.log" || fail "empty remote inventory was not reported distinctly"

: > "$TMP/remote-delete.log"; : > "$TMP/remote-prune.log"
if REMOTE_MODE=fail bash "$TMP/remote-prune-probe.sh"; then fail "failed remote inventory returned success"; fi
[[ ! -s "$TMP/remote-delete.log" ]] || fail "failed remote inventory triggered deletion"
grep -Fq 'Failed to list mockremote:vaultwarden_backups/db/ (rclone exit 17). No db files were deleted.' "$TMP/remote-prune.log" || fail "remote listing failure did not report path and no-delete outcome"
grep -Fq 'simulated listing failure' "$TMP/remote-prune.log" || fail "remote listing failure omitted actionable rclone error"
grep -Fq 'Remote retention did not complete' "$TMP/remote-prune.log" || fail "remote listing failure did not propagate nonzero summary"

printf 'PASS: canonical retention resolver and truthful local/remote pruning contracts\n'
)
...
ROOT="$VW_TEST_REPO_ROOT"
fail(){ echo "FAIL: $*" >&2; exit 1; }
_extract_func(){ local file="$1" func="$2"; awk -v f="$func" '$0 ~ "^" f "\\(\\)" {p=1} p {print; opens=gsub(/\{/,"{"); closes=gsub(/\}/,"}"); depth += opens-closes; if(depth==0) exit}' "$file"; }
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
...
get_config_value(){ printf '%s\n' vaultwarden; }
create_db_snapshot_host(){ return 1; }; verify_sqlite(){ return 1; }; _vaultwarden_container_running(){ return 0; }; wait_for_container_stopped(){ return 1; }
docker(){ printf '%s\n' "docker \$*" >> "$TMP/docker.calls"; }; sqlite3(){ return 1; }
...
prepare_case() {
  local case_dir="$1"
  mkdir -p "$case_dir/repo/utilities" "$case_dir/project/utilities" "$case_dir/state/data" "$case_dir/backups/db" "$case_dir/bin"
  ln -s "$ROOT/lib" "$case_dir/repo/lib"
  cp "$ROOT/utilities/maintenance-db-maint.sh" "$case_dir/repo/utilities/maintenance-db-maint.sh"
  sed -i.bak 's/^main "\$@"$/: # test harness: do not auto-run main/' "$case_dir/repo/utilities/maintenance-db-maint.sh"; rm -f "$case_dir/repo/utilities/maintenance-db-maint.sh.bak"
  printf 'SQLite format 3\000' > "$case_dir/state/data/db.sqlite3"
  cat > "$case_dir/project/utilities/backup-run.sh" <<'MOCK_BACKUP'
#!/usr/bin/env bash
set -euo pipefail
[[ "$*" == "run db" ]] || exit 90
if [[ "${BACKUP_BEHAVIOR:-success}" == "fail" ]]; then printf 'mock backup failure\n' >&2; exit 19; fi
...
#!/usr/bin/env bash
printf 'sqlite3 %s\n' "$*" >> "${ACTION_LOG:?}"
case "$*" in *integrity_check*) printf 'ok\n' ;; esac
exit 0
MOCK_SQLITE3
...
run_case() {
  ...
  (
    ...
    PROJECT_ROOT="$case_dir/project"; PROJECT_STATE_DIR="$case_dir/state"; BACKUP_DIR="$case_dir/backups"; DB_DEEP_FORCE=true; DRY_RUN=false; BACKUP_BEHAVIOR="$backup_behavior"; HEALTH_READY="$health_ready"; ACTION_LOG="$case_dir/actions.log"; PATH="$case_dir/bin:$PATH"
    export PROJECT_ROOT PROJECT_STATE_DIR BACKUP_DIR DB_DEEP_FORCE DRY_RUN BACKUP_BEHAVIOR HEALTH_READY ACTION_LOG PATH
    is_root(){ return 0; }; require_commands(){ return 0; }; require_docker(){ return 0; }; is_service_running(){ return 1; }; _wait_wal_quiesce(){ return 0; }; wait_for_service_ready(){ [[ "$HEALTH_READY" == "true" ]]; }
    run_deep_db_maintenance
  ) >"$output_file" 2>&1
}

failure_case="$TMP/backup-failure"; prepare_case "$failure_case"; : > "$failure_case/actions.log"
failure_rc=0; run_case "$failure_case" fail true "$failure_case/output" || failure_rc=$?
(( failure_rc != 0 )) || fail "--force deep maintenance continued after safety-backup failure"
grep -Fq 'Pre-maintenance safety backup failed; refusing deep database maintenance.' "$failure_case/output" || fail "backup failure did not report truthful fail-closed output"
[[ ! -s "$failure_case/actions.log" ]] || { cat "$failure_case/actions.log" >&2; fail "backup failure reached Docker or SQLite mutation under --force"; }

success_case="$TMP/success"; prepare_case "$success_case"
old_archive="$success_case/backups/db/db_backup_20000101_000000.sqlite3.age"
for suffix in '' .sha256 .sha256.hmac .meta; do printf 'older retained backup\n' > "${old_archive}${suffix}"; done
: > "$success_case/actions.log"
run_case "$success_case" success true "$success_case/output" || { cat "$success_case/output" >&2; fail "successful maintenance fixture failed"; }
new_archive="$success_case/backups/db/db_backup_20990101_000000.sqlite3.age"
for suffix in '' .sha256 .sha256.hmac .meta; do [[ ! -e "${new_archive}${suffix}" ]] || fail "successful maintenance retained temporary safety-backup member: ${new_archive}${suffix}"; [[ -e "${old_archive}${suffix}" ]] || fail "successful maintenance removed older retained backup member: ${old_archive}${suffix}"; done

incomplete_case="$TMP/incomplete"; prepare_case "$incomplete_case"; : > "$incomplete_case/actions.log"
incomplete_rc=0; run_case "$incomplete_case" success false "$incomplete_case/output" || incomplete_rc=$?
...
for suffix in '' .sha256 .sha256.hmac .meta; do [[ -e "${retained_archive}${suffix}" ]] || fail "incomplete maintenance removed safety-backup member: ${retained_archive}${suffix}"; done
grep -Fq 'Maintenance did not complete successfully. Retaining safety backup:' "$incomplete_case/output" || fail "incomplete maintenance did not report retained safety backup"
