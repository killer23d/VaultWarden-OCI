...
case "$MODE" in core|tail|all) ;; *) printf 'FAIL: unknown VW_TEST_CASE_MODE for case-restore-recovery.bash: %s\n' "$MODE" >&2; exit 2 ;; esac
...
run_test() {
    ...
    case "$MODE" in
        core)
            if (( _restore_recovery_run_test_index > _RESTORE_RECOVERY_MODE_SPLIT )); then
                _restore_recovery_run_test_impl \
                    "${name} # SKIP exercised by restore-tail logical mode" \
        ...
        tail)
            if (( _restore_recovery_run_test_index <= _RESTORE_RECOVERY_MODE_SPLIT )); then
                _restore_recovery_run_test_impl \
                    "${name} # SKIP exercised by restore-core logical mode" \
                    _restore_recovery_skip_test
                return
            fi
            ;;
...
case "$MODE" in
    ...
    all)
        ...
        ;;
    *)
        printf 'FAIL: unknown VW_TEST_CASE_MODE for case-restore-recovery.bash: %s\n' "$MODE" >&2
        exit 2
        ;;
esac
