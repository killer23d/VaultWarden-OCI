...
health_line="$(grep -n '_acquire_readonly_health_lock' <<< "$health_run_lock_block" | head -n1 | cut -d: -f1)"
global_line="$(grep -n 'operation_acquire' <<< "$health_run_lock_block" | head -n1 | cut -d: -f1)"
