...
check_permission_callers_are_thin() (
    ...
    [[ "$runtime_manager" == *'_vw_runtime_state_root "$operation" "$state_dir" || return 1'* ]] \
        || fail "unsafe runtime state roots do not abort before child inspection"
    [[ "$runtime_manager" != *'_vw_runtime_manage_tree "$operation" "$backup_dir/$path"'* ]] \
        || fail "backup child directories are still scanned recursively after the backup root"
    [[ "$runtime_manager" == *'install -d -m 750 -o "$puid" -g "$pgid" "$backup_dir/$path"'* ]] \
        || fail "backup child directories are not prepared without redundant recursive scans"

    pass "permission callers and runtime traversal remain single-pass"
)
