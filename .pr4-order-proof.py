...
replace_once(
    ...
    '''_firewall_managed_chain_order_is_safe() {\n    local line seen_drop=false\n    while IFS= read -r line; do\n        [[ "$line" == "-A ${VW_CF_DOCKER_CHAIN} "* ]] || continue\n        if [[ "$line" == *" -j DROP" ]]; then\n            seen_drop=true\n        elif [[ "$line" == *" -j RETURN" && "$seen_drop" == "true" ]]; then\n            return 1\n        fi\n    done < <(iptables -t filter -S "$VW_CF_DOCKER_CHAIN" 2>/dev/null)\n    return 0\n}\n\nfirewall_docker_ingress_is_exact() {\n''',
