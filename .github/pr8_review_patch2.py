...
if text.count(stale) != 1:
    raise SystemExit(f"utilities/backup-run.sh: expected one stale key hint, found {text.count(stale)}")
p.write_text(text.replace(stale, '            log_error "Restore the operational Age key at /etc/vaultwarden/age-key.txt, then re-run the backup."', 1))
...
old = r'''    cp "$ROOT/utilities/backup-run.sh" "$repo/utilities/backup-run.sh"
...
'''
new = r'''    cp "$ROOT/utilities/backup-run.sh" "$repo/utilities/backup-run.sh"
...
old_assert = r'''    [[ ! -e "$unit_dir" && ! -e "$opt_dir" && ! -e "$env_dir" ]] \
        || fail "dry-run mutated installation paths"
'''
new_assert = r'''    [[ ! -e "$unit_dir" && ! -e "$opt_dir" ]] \
...
# Lifecycle systemd install fixtures must satisfy the new preflight.
...
fixture_marker = r'''  mkdir -p "$unit_dir" "$opt_dir" "$env_dir" "$state" "$TMP/run-locks"
...
text = text.replace(
    fixture_marker,
    r'''  mkdir -p "$unit_dir" "$opt_dir" "$env_dir" "$state" "$TMP/run-locks"
...
# Runtime-authority suite: behavioral DNS proof and seam checks.
...
if text.count(var_marker) != 1:
    raise SystemExit(f"case-runtime-authority: setup-storage variable marker count {text.count(var_marker)}")
text = text.replace(
    var_marker,
