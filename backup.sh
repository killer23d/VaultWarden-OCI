...
preflight_checks() {
    ...
    # Check for service existence, not necessarily running status
    docker compose ps -a vaultwarden >/dev/null 2>&1 || { log_error "VaultWarden service not found in docker-compose.yml"; return 1; }
...
# Helper: Run integrity check using ephemeral container to avoid host sqlite3 dependency
verify_sqlite_integrity() {
    ...
    # Use dirname/basename to mount the specific directory
    local dir_name=$(dirname "$db_file")
    local file_name=$(basename "$db_file")
    
    # Run in alpine container
    local check_result
    check_result=$(docker run --rm -v "$dir_name:/check" alpine:latest \
        sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /check/$file_name '$check_cmd'" 2>&1) || return 1
        
    # Optimization is best-effort on snapshot
    docker run --rm -v "$dir_name:/check" alpine:latest \
        sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /check/$file_name 'PRAGMA optimize;'" >/dev/null 2>&1 || log_warn "Failed to optimize database snapshot, but continuing"
...
verify_backup_recoverability() {
    ...
    case "$backup_type" in
        "db")
            ...
            # Query table count via container
            local table_count
            table_count=$(docker run --rm -v "$verify_temp_dir:/check" alpine:latest \
                sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /check/test_recovery.db \"SELECT count(*) FROM sqlite_master WHERE type='table';\"" 2>/dev/null) || { log_error "CRITICAL: Cannot query recovered database!"; return 1; }
        ...
        "full"|"emergency")
            ...
            if age -d -i "$SOPS_AGE_KEY_FILE" "$encrypted_file" | tar -xzf - -C "$test_extraction_dir" ./data/db.sqlite3 2>/dev/null; then
                if [[ -f "$test_extraction_dir/data/db.sqlite3" ]]; then
                    ...
                    log_warn "Archive extracted but database file not found in expected location"; return 1; 
                fi
            else 
...
create_db_backup() {
    ...
    local timestamp backup_dir encrypted_file state_dir verification_mode
    ...
    state_dir="$(get_config_value "PROJECT_STATE_DIR" "/var/lib/vaultwarden")"; verification_mode="$(get_verification_mode)"
    
    # We use a temp file in the mounted volume so container can write to it and we can read it
    # Map: Host: $state_dir/data/backup_temp.db -> Container: /data/backup_temp.db
    local internal_temp="/data/backup_temp_${timestamp}.sqlite3"
    ...
    if is_service_running "vaultwarden"; then
        # Online backup via running container
        if ! docker compose exec -T vaultwarden sqlite3 /data/db.sqlite3 ".backup '$internal_temp'"; then
            log_error "Failed to create online backup via container"
            return 1
        fi
    else
        # Offline backup via ephemeral container
        log_info "VaultWarden stopped, using temporary container..."
        if ! docker run --rm -v "$state_dir/data:/data" alpine:latest \
            sh -c "apk add --no-cache sqlite >/dev/null && sqlite3 /data/db.sqlite3 \".backup '$internal_temp'\""; then
            log_error "Failed to create offline backup"
            return 1
        fi
    fi
    
    if [[ ! -f "$host_temp" ]]; then
        log_error "Backup file not found at expected path: $host_temp"
        return 1
    fi
    
    verify_sqlite_integrity "$host_temp" "$verification_mode" || { log_error "Snapshot integrity check failed, aborting backup"; return 1; }
    ...
    gzip -c "$host_temp" | encrypt_data > "$encrypted_file" || { log_error "Failed to compress and encrypt the snapshot"; return 1; }
...
create_archive_data() {
    local temp_dir="$1" verification_mode="$2" is_running="$3" state_dir="$4" timestamp="$5" include_secrets="$6"
    
    # Define locations for DB snapshot
    local internal_temp="/data/backup_full_temp.sqlite3"
    local host_temp="${state_dir}/data/backup_full_temp.sqlite3"
    local db_snapshot_dest="$temp_dir/data/db.sqlite3"
    
    log_info "Creating and verifying a consistent database snapshot..."
    
    # Snapshot logic (Containerized)
    ...
    if [[ "$snapshot_created" == "true" && -f "$host_temp" ]]; then
        verify_sqlite_integrity "$host_temp" "$verification_mode" || log_warn "Database integrity check failed, but continuing with archive."
    else
        log_warn "Failed to create live DB snapshot. Backup may be inconsistent or missing DB."
    fi
    ...
    if [[ -d "$state_dir/data" ]]; then
        ...
        # Exclude live database files
        rsync -a --delete --exclude 'db.sqlite3*' --exclude 'bwdata/db.sqlite3*' "$state_dir/data/" "$temp_dir/data/" 2>/dev/null || { log_warn "rsync failed, falling back to 'cp'..."; cp -a "$state_dir/data/." "$temp_dir/data/" || { log_error "Failed to copy data directory"; return 1; }; }
        
        # Move the verified snapshot into the archive structure
        if [[ -f "$host_temp" ]]; then
            mv "$host_temp" "$db_snapshot_dest" || { log_error "Failed to include database snapshot in archive"; return 1; }
...
create_full_backup() {
    ...
    create_archive_data "$temp_dir" "$verification_mode" "$is_running" "$state_dir" "$timestamp" "$include_secrets" || return 1
    
    [[ $DRY_RUN == true ]] && { log_info "[DRY-RUN] Would create and verify '$type' archive"; return 0; }
    
    log_info "Creating and encrypting '$type' archive..."; tar -czf - -C "$temp_dir" . | encrypt_data > "$encrypted_file" || { log_error "Failed to create or encrypt '$type' archive"; return 1; }
    secure_file "$encrypted_file" 600 || return 1
    
    local checksum; checksum=$(calculate_sha256 "$encrypted_file") || { log_error "Failed to calculate checksum"; return 1; }
    echo "$checksum" > "$encrypted_file.sha256"
    verify_post_encryption_checksum "$encrypted_file" || { log_error "CRITICAL: $type backup created but failed post-encryption verification!"; return 1; }
    verify_backup_recoverability "$encrypted_file" "$type" || { log_error "CRITICAL: $type backup created but failed recoverability verification!"; return 1; }
    generate_metadata "$encrypted_file" "$type" "$timestamp" "$checksum" || log_warn "Failed to create metadata, but backup is valid"
    log_success "$type backup created and verified: $(basename "$encrypted_file")"
...
main() {
    ...
    local backup_file=""
    case "$BACKUP_TYPE" in db) backup_file="$(create_db_backup)" ;; full) backup_file="$(create_full_backup 'full')" ;; emergency) backup_file="$(create_full_backup 'emergency')" ;; *) log_error "Unknown backup type: $BACKUP_TYPE"; exit 1 ;; esac
