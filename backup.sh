...
preflight_checks() {
    ...
    # Check for service existence, not just running state
    ...
    # NEW: Verify Age Key Health before starting (Resilience Check)
...
# Helper: Run SQLite command inside container (Host dependency removal)
# Mounts the directory containing the DB file to a temporary container
run_sqlite_containerized() {
    local db_file="$1"
    local sql_cmd="$2"
    ...
    # Use alpine with sqlite to run the check on the mounted file
    docker run --rm -v "$dir_name:/check" alpine:latest \
        sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /check/$file_name \"$sql_cmd\""
}
...
verify_sqlite_integrity() {
    ...
    local check_result
    # MODIFIED: Use containerized execution
    check_result=$(run_sqlite_containerized "$db_file" "$check_cmd" 2>&1) || return 1
    
    # Attempt optimization (best effort)
    run_sqlite_containerized "$db_file" "PRAGMA optimize;" >/dev/null 2>&1 || log_warn "Failed to optimize database, but continuing"
...
verify_backup_recoverability() {
    ...
    case "$backup_type" in
        "db")
            ...
            # MODIFIED: Containerized table count check
            local table_count
            table_count=$(run_sqlite_containerized "$test_db" "SELECT count(*) FROM sqlite_master WHERE type='table';" 2>/dev/null) || { log_error "CRITICAL: Cannot query recovered database!"; return 1; }
...
create_db_backup() {
    ...
    # Use a temp file inside the mounted data volume for container access
    local temp_filename="backup_temp_${timestamp}.sqlite3"
    local host_temp_snapshot="${state_dir}/data/${temp_filename}"
    local container_temp_path="/data/${temp_filename}"
    register_cleanup "rm -f '$host_temp_snapshot'"
    
    log_info "Creating and verifying a consistent database snapshot..."
    
    # MODIFIED: Use Containerized sqlite3
    if [[ -f "$db_file" ]]; then
        if is_service_running "vaultwarden"; then
            # Online: Use running container (mounted volume)
            if docker compose exec -T vaultwarden sqlite3 /data/db.sqlite3 ".backup '$container_temp_path'"; then
                log_info "Snapshot created successfully using container exec"
            else
                log_error "Failed to create snapshot via container exec"
                return 1
            fi
        else
            # Offline: Use ephemeral container (mount volume)
            if docker run --rm -v "$state_dir/data:/data" alpine:latest \
                sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /data/db.sqlite3 \".backup '$container_temp_path'\""; then
                log_info "Snapshot created successfully using temp container"
            else
                log_error "Failed to create snapshot via temp container"
                return 1
            fi
        fi
        
        # Verify the snapshot exists on host
        if [[ ! -f "$host_temp_snapshot" ]]; then
            log_error "Snapshot file missing on host at: $host_temp_snapshot"
            return 1
        fi
        
        verify_sqlite_integrity "$host_temp_snapshot" "$verification_mode" || { log_error "Snapshot integrity check failed, aborting backup"; return 1; }
    else
    ...
    # Note: compressing from the host temp path
    gzip -c "$host_temp_snapshot" | encrypt_data > "$encrypted_file" || { log_error "Failed to compress and encrypt the snapshot"; return 1; }
...
create_archive_data() {
    ...
    local temp_filename="db.sqlite3.snapshot.${timestamp}"
    local host_db_snapshot="${state_dir}/data/${temp_filename}"
    local container_db_snapshot="/data/${temp_filename}"
    ...
    if [[ -f "$db_file" ]]; then
         if [[ "$is_running" == "true" ]]; then
             if docker compose exec -T vaultwarden sqlite3 /data/db.sqlite3 ".backup '$container_db_snapshot'"; then
                 log_info "Snapshot created successfully using container exec"
             else
                 log_warn "Failed to create live DB snapshot. Copying via host as fallback."
                 cp "$db_file" "$host_db_snapshot"
             fi
         else
             # Offline fallback container
             if docker run --rm -v "$state_dir/data:/data" alpine:latest \
                sh -c "apk add --no-cache sqlite >/dev/null 2>&1 && sqlite3 /data/db.sqlite3 \".backup '$container_db_snapshot'\""; then
                 log_info "Snapshot created successfully using temp container"
             else
                 log_warn "Failed to create offline DB snapshot. Copying via host as fallback."
                 cp "$db_file" "$host_db_snapshot"
             fi
         fi
         
         # Cleanup snapshot file when done
         register_cleanup "rm -f '$host_db_snapshot'"
         
         verify_sqlite_integrity "$host_db_snapshot" "$verification_mode" || log_warn "Database integrity check failed, but continuing with archive."
    else 
    ...
    if [[ -d "$state_dir/data" ]]; then
        ...
        # Exclude both bwdata/db.sqlite3 and db.sqlite3 just in case, PLUS our temporary snapshot file
        rsync -a --delete \
            --exclude 'bwdata/db.sqlite3*' \
            --exclude 'db.sqlite3*' \
            --exclude "db.sqlite3.snapshot.*" \
            "$state_dir/data/" "$temp_dir/data/" 2>/dev/null || { 
                log_warn "rsync failed, falling back to 'cp'..."
                cp -a "$state_dir/data/." "$temp_dir/data/" || { log_error "Failed to copy data directory"; return 1; }
            }
        
        # Ensure DB snapshot is in the right place (root of data dir or bwdata if needed)
        if [[ -f "$host_db_snapshot" ]]; then 
            # Move our verified snapshot into the archive structure
            # Standard path for this setup seems to be /data/db.sqlite3
            mv "$host_db_snapshot" "$temp_dir/data/db.sqlite3" || { log_error "Failed to include database snapshot in archive"; return 1; }
