...
# ENHANCED: Key resilience verification + Containerized SQLite operations
# FIXED: Removes dependency on host-side sqlite3 binary
...
create_db_backup() {
    ...
    # We use a temp file INSIDE the mounted volume so both container and host can see it
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        log_info "[DRY-RUN] Would backup DB via container exec"
        return 0
    fi
    
    log_info "Creating consistent snapshot via container..."
    if is_service_running "vaultwarden"; then
        # Online backup: execute inside running container
        ...
        # Offline backup: execute via ephemeral container
    ...
    if [[ ! -f "$host_temp" ]]; then
        log_error "Backup file not found at expected host path: $host_temp"
        return 1
    fi
...
create_archive_data() {
    ...
    # 4. DB Snapshot (Containerized)
    local internal_temp="/data/backup_full_temp.sqlite3"
    local host_temp="${state_dir}/data/backup_full_temp.sqlite3"
    
    # Manual cleanup needed later since we move the file
    log_info "Snapshotting database for archive..."
    if is_service_running "vaultwarden"; then
        docker compose exec -T vaultwarden sqlite3 /data/db.sqlite3 ".backup '$internal_temp'"
    else
        docker run --rm -v "$state_dir/data:/data" alpine:latest \
            sh -c "apk add --no-cache sqlite >/dev/null && sqlite3 /data/db.sqlite3 \".backup '$internal_temp'\""
    fi
    
    if [[ -f "$host_temp" ]]; then
        mv "$host_temp" "$temp_dir/data/db.sqlite3"
    else
        log_warn "Database snapshot failed, archive will lack database!"
    fi
}
