...
_is_project_owned_unit_artifact() {
    ...
    # Top-level project-namespace symlinks are always project-owned inventory
    # entries. Never follow their target to classify them.
    [[ -L "$path" ]] && return 0
    [[ -f "$path" ]] || return 1
...
_list_unexpected_managed_artifacts() {
    ...
    if [[ -d "$UNIT_DEST_DIR" ]]; then
        while IFS= read -r -d '' path; do
            ...
            if [[ -L "$path" ]]; then
                printf 'unit|%s\n' "$path"
            elif ! _inventory_contains "$unit" "${_MANAGED_UNIT_FILES[@]}" \
               && _is_project_owned_unit_artifact "$path"; then
                printf 'unit|%s\n' "$path"
            fi
        done < <(
            find "$UNIT_DEST_DIR" -mindepth 1 -maxdepth 1 \
                \( -type f -o -type l \) \
...
_reload_runtime_environment_after_sync() {
    ...
    log_success "Reloaded canonical runtime configuration from ${VW_CONFIG_SELECTED_ENV_FILE}"
}
