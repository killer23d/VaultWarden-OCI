...
install_cron_jobs() {
    ...
    local cron_jobs=(
        ...
        "0 4 * * 1-6 cd $PROJECT_ROOT || exit 1; $CRON_SCRIPTS_DIR/backup.sh --type db --rclone --email >> $CRON_LOG_DIR/db-backup.log 2>&1"
        ...
        "0 3 * * 0 cd $PROJECT_ROOT || exit 1; $CRON_SCRIPTS_DIR/backup.sh --type full --full-verification --rclone --email >> $CRON_LOG_DIR/full-backup.log 2>&1"
