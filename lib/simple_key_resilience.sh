...
simple_verify_age_key() {
    ...
    # We need to extract the public key first to encrypt
    local public_key
    ...
    # Perform round-trip test to ensure key actually works
...
create_password_manager_escrow() {
    ...
    # Create a self-contained backup file with instructions
    cat > "$output_file" << EOF
═══════════════════════════════════════════════════════════════
...
EOF
    ...
    # Convert to PDF if tool available, else keep HTML
