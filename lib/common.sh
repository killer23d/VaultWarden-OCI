#!/usr/bin/env bash
# lib/common.sh — Facade loader and core utility functions for VaultWarden-OCI.
# This file is the single source entrypoint for all scripts. It loads domain
# libraries and provides core utilities that do not belong to any single domain.
# Loaded domain libraries:
#   lib/log.sh      — Logging, colour constants, log-level filtering
#   lib/validate.sh — Pure input-validation functions (email, domain, port, ip, url)
#   lib/config.sh   — Secure .env parsing and configuration state management
# Core utilities retained here:
#   Privilege    : is_root, require_root, get_real_user, _maybe_sudo,
#                  auto_fix_critical_permissions
#   System       : has_command, require_commands, retry_with_backoff, _require_script
...
# ---------------------------------------------------------------------------
# Modular library loading — common.sh acts as a facade that loads all domain
# libraries. Scripts sourcing common.sh get the full stack automatically.
# Lightweight scripts may source individual lib files directly (e.g. lib/log.sh).
# ---------------------------------------------------------------------------
...
LOG_PREFIX=""
_VW_CALLING_SCRIPT=""
# shellcheck disable=SC2034  # referenced by log.sh and sourced scripts
LOG_TIMESTAMP=true
# shellcheck disable=SC2034  # referenced by log.sh and sourced scripts
LOG_COLORS=true
...
init_common_lib() {
    ...
    # ---------------------------------------------------------------------------
    # Architecture detection — sets HOST_ARCH and GITHUB_ARCH for use by any
    # script that sources common.sh. Two variables are intentional:
    #
    #   HOST_ARCH   — dpkg-canonical name used by apt sources and Docker manifests
    #                 (values: amd64, arm64, armhf)
    #   GITHUB_ARCH — binary release naming convention used by GitHub release
    #                 assets (values: amd64, arm64, arm)
    #
    # They differ because GitHub release filenames for tools like SOPS and age
    # use "arm" for armhf, not "armhf". Without the split, downloading
    # sops-vX.Y.Z.linux.armhf would silently 404 instead of fetching .linux.arm.
    # ---------------------------------------------------------------------------
    local _raw_arch
    _raw_arch=$(dpkg --print-architecture 2>/dev/null || uname -m)

    case "$_raw_arch" in
        amd64|x86_64)   HOST_ARCH="amd64"  ;;
        arm64|aarch64)  HOST_ARCH="arm64"  ;;
        armhf|armv7l)   HOST_ARCH="armhf"  ;;
        *)              HOST_ARCH="$_raw_arch" ;;
    esac
    export HOST_ARCH

    case "$HOST_ARCH" in
        amd64)  GITHUB_ARCH="amd64"  ;;
        arm64)  GITHUB_ARCH="arm64"  ;;
        armhf)  GITHUB_ARCH="arm"    ;;
        *)      GITHUB_ARCH="$HOST_ARCH" ;;
    esac
    export GITHUB_ARCH

    log_debug "Architecture: HOST_ARCH=${HOST_ARCH} GITHUB_ARCH=${GITHUB_ARCH}"
