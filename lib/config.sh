#!/usr/bin/env bash
# lib/config.sh — Secure .env file parsing and configuration state management
#                 for VaultWarden-OCI.
# Provides:
#   load_env_file, get_config_value, require_config
#   _set_env_var, _read_env_value, _get_file_perms
# Security model:
#   - load_env_file guards against shell injection ($(...) and backtick forms)
#   - Refuses to overwrite PATH, LD_PRELOAD, LD_LIBRARY_PATH, IFS, BASH_ENV,
#     ENV, CDPATH, PS4 from an untrusted file
#   - Enforces 600 permissions when running as root; warns otherwise
# Dependencies: lib/log.sh  (uses log_* for all output)
# Sourced by: lib/common.sh (facade)

[[ -n "${VW_CONFIG_LIB_LOADED:-}" ]] && return 0
readonly VW_CONFIG_LIB_LOADED=1

# config.sh sources log.sh directly so it can be used standalone without
# loading the full common.sh facade.
_VW_CONFIG_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${_VW_CONFIG_LIB_DIR}/log.sh"

# Derive project root when common.sh has not already defined PROJECT_ROOT.
_VW_CONFIG_PROJECT_ROOT="${PROJECT_ROOT:-$(cd "${_VW_CONFIG_LIB_DIR}/.." && pwd)}"

_ENV_FILE_SEARCH_PATHS=(
    "${_VW_CONFIG_PROJECT_ROOT}/.env"
...
export -f _get_file_perms load_env_file get_config_value require_config _set_env_var _read_env_value
