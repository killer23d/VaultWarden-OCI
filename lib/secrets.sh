...
# ensure_sops_env
# ---------------------------------------------------------------------------
# Single canonical function to export both SOPS env vars from an absolute
# path.  All scripts and internal functions must call this instead of
# directly exporting SOPS_AGE_KEY_FILE / SOPS_CONFIG so that there is one
# place to change if paths ever move.
#
# Usage:
#   ensure_sops_env                        # uses AGE_KEY_FILE and PROJECT_ROOT
#   ensure_sops_env "/abs/path/age-key"   # explicit key path override
# ---------------------------------------------------------------------------
ensure_sops_env() {
...
# ---------------------------------------------------------------------------
# Compatibility shims so callers of the old names still work
# ---------------------------------------------------------------------------
# setup_secrets_environment was the old name in setup-secrets.sh / secrets.sh
setup_secrets_environment() { ensure_sops_env "${1:-}"; }

# cleanup_secrets_environment is intentionally a no-op: SOPS vars must
# persist for the lifetime of the calling script.
...
# list_secret_keys
# ---------------------------------------------------------------------------
# Print the top-level YAML keys present in the encrypted secrets file
# WITHOUT printing any values.  Useful for ops auditing.
#
# Usage: list_secret_keys [secrets_file]
# ---------------------------------------------------------------------------
list_secret_keys() {
    local secrets_file="${1:-$SECRETS_FILE}"
...
# Create backup of secrets file (timestamped)
...
# ---------------------------------------------------------------------------
# Prune old secrets.yaml backups keeping only the N most recent.
#
# NOTE: This function is intentionally SEPARATE from backup_utils.sh's
# cleanup_old_backups(), which operates on day-based retention and expects
# *.age encrypted backup files.  This function works on count-based
# retention and plain secrets.yaml.backup-* files.
#
# Usage: cleanup_old_secret_backups [backup_dir] [keep_count]
# ---------------------------------------------------------------------------
cleanup_old_secret_backups() {
