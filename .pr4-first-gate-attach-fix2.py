...
late = re.compile(
    ...
    r'        return 1\n'
    r'    \}\n\n', re.S)
t, n = late.subn('', t, count=1)
if n != 1:
    raise SystemExit(f'late established/jump block matches: {n}')
cf = re.compile(
    r'(iptables -t filter -I "\$VW_CF_DOCKER_CHAIN") 1 (\\\n\s+-d "\$VW_CADDY_EXTERNAL_CIDR" -s "\$cidr" -p tcp -m conntrack --ctorigdstport "\$port" -j RETURN)'
)
t, n = cf.subn(r'\1 2 \2', t, count=1)
if n != 1:
    raise SystemExit(f'Cloudflare RETURN insertion matches: {n}')
p.write_text(t)
...
'''
extra = '''sentinel80_line="$(grep -n 'iptables -t filter -I VW-CF-INGRESS 1 -d 172.22.0.0/28 -p tcp -m conntrack --ctorigdstport 80 -j DROP' "$IPT_CALL_LOG" | cut -d: -f1 | head -1)"
...
'''
if anchor not in t:
    raise SystemExit('initial gate test anchor missing')
...
'''
if anchor not in t:
    raise SystemExit('orphan stale test anchor missing')
p.write_text(t.replace(anchor, anchor + extra, 1))
