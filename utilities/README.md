...
Lists all secret key names in `$SECRETS_FILE` without decrypting any values.
...
Decrypts and displays `$SECRETS_FILE` in a read-only pager. No changes
...
`$SECRETS_FILE`, and resyncs Docker secret bind-mount files.
