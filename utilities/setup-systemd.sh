...
_sync_runtime_environment_files() {
    if [[ "$DRY_RUN" == "true" ]]; then
        log_info "[DRY RUN] Would run utilities/sync-env.sh to regenerate install.env and $ENV_FILE"
        return 0
    fi

    log_info "Regenerating runtime environment files via sync-env..."
    "${PROJECT_ROOT}/utilities/sync-env.sh" || return 1
    log_success "Runtime environment files regenerated by sync-env"
}
