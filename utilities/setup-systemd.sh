...
install_units() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        if [[ -f "$age_key_src" ]]; then
            ...
            log_info "[DRY RUN] env-edit sync would set SOPS_AGE_KEY_FILE=$AGE_KEY_DEST in generated runtime env files"
        else
            log_warn "[DRY RUN] Age key source not found: $age_key_src"
            if [[ -f "$AGE_KEY_DEST" ]]; then
                log_info "[DRY RUN] Key already at $AGE_KEY_DEST -- env-edit sync would correct SOPS_AGE_KEY_FILE"
            else
                log_warn "[DRY RUN] Install the key, then run env-edit sync to refresh generated runtime env files."
            fi
        fi
    else
        if [[ -f "$age_key_src" ]]; then
    ...
    if [[ -f "$rclone_dest" ]]; then
        ...
        if [[ -n "$rclone_src" ]]; then
            if [[ "$DRY_RUN" == "true" ]]; then
                ...
                log_info "[DRY RUN] env-edit sync would set RCLONE_CONFIG=$rclone_dest in generated runtime env files"
            else
