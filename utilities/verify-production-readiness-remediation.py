...
setup_impl_text = setup_text + "\n" + (root / "setup-main.sh").read_text()
checks = {
    "setup protected handoff": "VWOCI-PRR-PATCH-01" in handoff_text,
    "required UFW failure": "Required UFW firewall configuration failed" in setup_impl_text,
    "required auto-secrets failure": "Required automatic secrets configuration failed" in setup_impl_text,
    ...
    "no recursive log 0755": not re.search(r"chmod\s+-R\s+755.*logs", setup_impl_text),
    ...
    "retired backup passphrase absent from setup": "BACKUP_PLAIN_FILE" not in setup_impl_text and "04  BACKUP PASSPHRASE" not in handoff_text,
