...
check_age_key_health_for_update() {
    ...
    if ! check_age_key_health 2>/dev/null; then
        ...
        log_warn "  1. Selected operational key is missing or points to the wrong installed path"
        log_warn "     Inspect with: sudo make info"
        log_warn "  2. Wrong key ownership or mode (expected root-owned 600 in production)"
